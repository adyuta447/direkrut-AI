...
function developmentAreaFor(jobTitle: string): string {
  ...
  if (title.includes("developer") || title.includes("engineer") || title.includes("backend") || title.includes("frontend")) {
    return "penguasaan arsitektur sistem skala besar, pengujian otomatis (unit & integration testing), dan praktik DevOps/CI-CD";
  }
  if (title.includes("keuangan") || title.includes("finance") || title.includes("akuntansi")) {
...
export function buildRejectionEmail(candidate: Application): string {
  const score = candidate.recommendationScore || 0;
  const title = candidate.jobTitle || "";
  const areaKembangkan = developmentAreaFor(title);
